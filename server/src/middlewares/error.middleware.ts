 ...
 */
const handleValidationError = (err: mongoose.Error | Error): AppError | null => {
 ...
 */
const handleCastError = (err: mongoose.Error | Error): AppError | null => {
...
const handleDuplicateError = (err: MongoError): AppError | null => {
  if (err.code === 11000 && err.keyValue && Object.keys(err.keyValue).length > 0) {
    const field = Object.keys(err.keyValue)[0];
    const value = err.keyValue[field];
    return AppError.conflict(
      `Duplicate field value: ${field}. Please use another value for ${field}: ${value}`
    );
  }
  return null;
};
